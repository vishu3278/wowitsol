...
// print_r($_POST); die;
	...
	if(!empty($title) && !empty($status)  && !empty($description)   ){
			...
			$qry = "INSERT INTO  news (b_title,b_url_key,b_description,b_featured_image,b_status,b_publish_date,b_edit_date)  VALUES('".$title."','".$b_url_key."','".addslashes($description)."','".$b_featured_image."','".$status."','".$created_date."','".$modifieddate."')" ;
				
				$sql = mysqli_query($conn, $qry);

				if($sql){

				echo$b_id = mysqli_insert_id();
				
				if($status == 'publish') {
					$_SESSION['success'] = 'Your news has been published ';
				}else{
