...
// print_r($_POST); die;
	...
	if(!empty($title) && !empty($status)  && !empty($description)   ){
			...
			$qry = "UPDATE  blog  SET  b_title= '".$title."',  b_description = '".addslashes($description)."',  b_featured_image = '".$b_featured_image."',  b_status = '".$status."',   b_edit_date = '".$modifieddate."' WHERE  b_id = '".$b_id."'" ;
				...
				if($status == 'publish') {
					...
					$_SESSION['success'] = 'Your blog has been Edited Successfully ';
